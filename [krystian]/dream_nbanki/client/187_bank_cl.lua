...
RegisterNetEvent('187_banki:currentlyrobbing')
AddEventHandler('187_banki:currentlyrobbing', function(robb)
	holdingup = true
	store = robb
end)

RegisterNetEvent('187_banki:killblip')
AddEventHandler('187_banki:killblip', function()
	
    RemoveBlip(blipRobbery)
end)

RegisterNetEvent('187_banki:setblip')
AddEventHandler('187_banki:setblip', function(position)
...
RegisterNetEvent('187_banki:toofarlocal')
AddEventHandler('187_banki:toofarlocal', function(robb)
	holdingup = false
...
RegisterNetEvent('187_banki:robberycomplete')
AddEventHandler('187_banki:robberycomplete', function(robb)
	holdingup = false
...
Citizen.CreateThread(function()
	...
	while true do
							...
							if IsPedArmed(PlayerPedId(), 4) then
								if Config.NeedBag then
									if borsa == 40 or borsa == 41 or borsa == 44 or borsa == 45 then
										ESX.TriggerServerCallback('187_banki:conteggio', function(CopsConnected)
											if CopsConnected >= Config.RequiredCopsRob then
												TriggerServerEvent('187_banki:rob', k)
											else
		...
		if holdingup then
				if notka then
					local data = {displayCode = '10-90', description = store2.nameofstore, isImportant = 0, recipientList = {'police'}, length = '15000', infoM = 'fas fa-piggy-bank', info = "Ktoś aktywował alarm w banku.", blipSprite = 161, blipScale = 0.9, blipColour = 1}
					local dispatchData = {dispatchData = data, caller = "DYSPOZYTORNIA", coords = vector3(xx, yy, zz)}
					TriggerServerEvent('wf-alerts:svNotify', dispatchData)
					notka = false
				end
				if(GetDistanceBetweenCoords(pos, Config.SmallBanks[closestBank]["coords"]["x"], Config.SmallBanks[closestBank]["coords"]["y"], Config.SmallBanks[closestBank]["coords"]["z"], true) < 10.0) and not Config.SmallBanks[closestBank]["coords"]["open"] and Config.EnableMarker then 
				...
				if(GetDistanceBetweenCoords(pos, Config.SmallBanks[closestBank]["coords"]["x"], Config.SmallBanks[closestBank]["coords"]["y"], Config.SmallBanks[closestBank]["coords"]["z"], true) < 0.75) and not Config.SmallBanks[closestBank]["coords"]["open"] then 
					...
					incircle = true
					if IsControlJustPressed(0, 38) then
						ESX.TriggerServerCallback('187_banki:getInvItem', function(has_hackerDevice)
						if has_hackerDevice then
							...
							SetEntityHeading(GetPlayerPed(-1), Config.SmallBanks[closestBank]["heading"].heading) -- ustawia gracza skierowanego w dane kordy
											...
											Skillbar.Start({
												...
												Skillbar.Start({
													...
													Skillbar.Start({
														...
														TriggerServerEvent('187_banki:drzwi', closestBank)
														rapina = false
														standrzwi = true
													end, function()
														ClearPedTasksImmediately(GetPlayerPed(-1))
														...
														Config.SmallBanks[closestBank]["coords"]["open"] = false
															ESX.TriggerServerCallback('187_banki:removeInvItem', function(has_hackerDevice)
																if has_hackerDevice then 
							...
							TriggerEvent('pNotify:SendNotification', {text = 'Nie posiadasz Urządzenia Hackerskiego!', type = 'success', layout = 'centerRight'})
						end
						end, 'hackerDevice' , 1)
				end
			...
			if fjut then
				...
				for i,v in pairs((Config.SmallBanks[closestBank]["lockers"])) do 
					...
					if(GetDistanceBetweenCoords(pos, Config.SmallBanks[closestBank]["lockers"][i].x, Config.SmallBanks[closestBank]["lockers"][i].y, Config.SmallBanks[closestBank]["lockers"][i].z, true) < 0.75) and not Config.SmallBanks[closestBank]["lockers"][i]["isOpened"] then 
						...
						if IsControlJustPressed(0, 38) then
							ESX.TriggerServerCallback('187_banki:getInvItem', function(has_drill)
								if has_drill then
								...
								TriggerEvent("Drilling:Start",function(drillStatus)
									if (drillStatus == 1) then
										...
										TriggerServerEvent('187_baki:reset', work, skrytkiRotte)
										work = false
										if skrytkicheck == 8 then 
											...
											holdingup = false
											fjut = false
											TriggerServerEvent('187_banki:sstop')
											TriggerServerEvent('187_banki:endrob', store)
											Citizen.Wait(3000)
											...
											Citizen.Wait(30000)
											TriggerServerEvent('187_banki:rdrzwi')
										end
									elseif (drillStatus == 3) then
										Config.SmallBanks[closestBank]["lockers"][i]["isOpened"] = true
										skrytkicheck = skrytkicheck+1
										FreezeEntityPosition(player, false)
											ESX.TriggerServerCallback('187_banki:removeInvItem', function(has_drill)
												if has_drill then 
								...
								TriggerEvent('pNotify:SendNotification', {text = 'Nie posiadasz Wiertła!', type = 'success', layout = 'centerRight'})
							end
							end, 'drill' , 1)
						end
				end
			...
			if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), pos2.x, pos2.y, pos2.z, true) > 10.5 ) then
				TriggerServerEvent('187_banki:toofar', store)
				holdingup = false
				fjut = false
				...
				TriggerServerEvent('187_banki:sstop')
				TriggerServerEvent('esx_vangelico_robbery:toofa')
				rstandrzwi = true
				StopSound(soundid2)
				Citizen.Wait(3000)
					if standrzwi then 
...
RegisterNetEvent('187_banki:stopSound')
AddEventHandler('187_banki:stopSound', function()
    StopSound(soundid)
end)

RegisterNetEvent('notificationns')
AddEventHandler('notificationns', function()
    TriggerServerEvent('187_banki:rdrzwi')
end)
...
AddEventHandler('187_banki:drzwi', function(bankId)
...
AddEventHandler('187_banki:rdrzwi', function()
    for k, v in pairs(Config.SmallBanks) do
        local object = GetClosestObjectOfType(Config.SmallBanks[k]["coords"]["x"], Config.SmallBanks[k]["coords"]["y"], Config.SmallBanks[k]["coords"]["z"], 5.0, Config.SmallBanks[k]["object"], false, false, false)
        if not Config.SmallBanks[k]["isOpened"] then
            SetEntityHeading(object, Config.SmallBanks[k]["heading"].closed)
