...
RegisterNetEvent('187:tajming')
AddEventHandler('187:tajming', function(tajmus)
...
Citizen.CreateThread(function()
	...
	--										TriggerEvent('esx:showNotification', _U('min_two_police') .. Config.RequiredCopsRob .. _U('min_two_police2'))
		...
		if holdingup then
			if notka1 then
				local data = {displayCode = '10-90', description = store22.nameofstore, isImportant = 0, recipientList = {'police'}, length = '15000', infoM = 'fas fa-gem', info = "Ktoś aktywował alarm w jubilerze.", blipSprite = 161, blipScale = 0.9, blipColour = 1}
				local dispatchData = {dispatchData = data, caller = "DYSPOZYTORNIA", coords = vector3(xxX, yyY, zzZ)}
				TriggerServerEvent('wf-alerts:svNotify', dispatchData)
				notka1 = false
			end
			drawTxt(0.3, 1.4, 0.30, _U('smash_case') .. ' :~r~ ' .. vetrineRotte .. '/' .. Config.MaxWindows, 185, 185, 185, 255)

			for i,v in pairs(vetrine) do 
				if(GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 5.0) and not v.isOpen and Config.EnableMarker then 
					DrawMarker(20, v.x, v.y, v.z, 0, 0, 0, 0, 0, 0, 0.6, 0.6, 0.6, 16, 145, 204, 200, 1, 1, 0, 0)
				end
				if(GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 0.75) and not v.isOpen then 
					...
					if IsControlJustPressed(0, 38) then
						...
						TriggerServerEvent('esx_vangelico_robbery:gioielli', work, vetrineRotte)
					    animazione = false
						work = false
			...
			if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -622.566, -230.183, 38.057, true) > 11.5 ) then
				...
				TriggerServerEvent('sstop')
				TriggerEvent('pNotify:SendNotification', {text = 'Napad przerwany na Jubilera!', type = 'success', layout = 'centerRight'})
			end
